...
How useful is this skill to agent users? Use weighted sub-scores, then map to a final 1-5 score.

`weighted-v2` is the only supported usage scoring method. Legacy single-score-only usage inputs are not supported.

### Sub-Scores (1-5 each)

Record these five sub-scores under `usage_value.sub_scores`:

| Sub-Score | What to Assess | Weight |
|-----------|----------------|--------|
| `problem_clarity` | Is the user problem explicit, concrete, and outcome-oriented? | 0.25 |
| `audience_breadth` | How many users or teams plausibly need this? | 0.20 |
| `capability_gap` | How much unique value does this add over baseline model behavior? | 0.25 |
| `actionability` | Does the skill produce directly usable outputs/workflows? | 0.20 |
| `reusability` | Does it generalize across contexts without hardcoded assumptions? | 0.10 |

### Calculation

1. Compute weighted score:

	`weighted_score = Σ(sub_score × weight)`

2. Apply domain calibration bonus:

	`final_raw = weighted_score + domain_calibration_bonus`

	Where `domain_calibration_bonus` is in `[0.0, 0.7]`.

3. Clamp and round:

	`score = round(clamp(final_raw, 1.0, 5.0))`

Record all four fields:
- `usage_value.score`
- `usage_value.sub_scores`
- `usage_value.weighted_score`
- `usage_value.domain_calibration_bonus`

### Domain Calibration Guidance

Use a positive bonus when the skill addresses high-gap domains where baseline models are weaker (for example legal, compliance, behavioral science, insurance, healthcare, regulated operations). Typical range: `+0.3` to `+0.7`.

Use `0.0` for general domains with low specialization gap.

### Consistency Guard

If rationale says the skill is "domain-specific" or "niche" and the computed/assigned score is below 3, raise to 3 unless there is explicit evidence of low quality.

### Interpretation Bands

| Score | Interpretation |
|-------|----------------|
| **5** | Essential and high-leverage for broad or high-impact specialized usage |
| **4** | Strong, clear value for a significant segment |
| **3** | Niche but solid and useful |
| **2** | Narrow value or weak differentiation |
| **1** | Minimal value or unclear purpose |
