...
Usage scoring method note: `evaluation.usage_value.method_version` must be `weighted-v2`. Legacy usage scoring modes are unsupported.
...
| `usage_value.sub_scores.problem_clarity` | integer | 1-5 |
| `usage_value.sub_scores.audience_breadth` | integer | 1-5 |
| `usage_value.sub_scores.capability_gap` | integer | 1-5 |
| `usage_value.sub_scores.actionability` | integer | 1-5 |
| `usage_value.sub_scores.reusability` | integer | 1-5 |
| `usage_value.weighted_score` | float | Weighted pre-bonus usage score |
| `usage_value.domain_calibration_bonus` | float | Domain bonus in range 0.0-0.7 |
| `usage_value.method_version` | string | Scoring method tag (for example `weighted-v2`) |
